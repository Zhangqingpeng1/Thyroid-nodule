...
def make_dataloader():
    df = create_dataframe_from_directory(r'/data/zqp/XR/data/DDTI_ture/image', r'/data/zqp/XR/data/DDTI_ture/mask')
...
if __name__ == '__main__':
    ...
    model_0 = PetModel("FPN", "resnet34", in_channels=3, out_classes=1, lr=0.001)
    trainer = Trainer(accelerator="gpu", max_epochs=120, devices=[0])
    trainer.fit(model_0,
                train_dataloaders=train_dataloader,
                val_dataloaders=val_dataloader, )
    trainer.test(model_0, dataloaders=test_dataloader, verbose=False)
    with open(f'/data/zqp/XR/DDTI/plus_{model_0.archs}_loss.pkl', 'wb') as file:   
        pickle.dump(model_0.loss_list, file)    
    with open(f'/data/zqp/XR/DDTI/plus_{model_0.archs}_metrics.pkl', 'wb') as file:  
        pickle.dump(model_0.total_metrics, file)
    torch.save(model_0.state_dict(), f'/data/zqp/XR/DDTI/plus_weight_{model_0.archs}.pth') 