...
class PetModel(LightningModule):
    def __init__(self, arch, encoder_name, in_channels, out_classes, lr, weight_decay=0, **kwargs):
        ...
        self.model = smp.create_model(
            arch, encoder_name=encoder_name, in_channels=in_channels, classes=out_classes, encoder_weights=None,
            **kwargs
        )
        self.archs = arch
        self.val_loss = 10000
...
if __name__ == '__main__':
    ...
    trainer = Trainer(accelerator="gpu", max_epochs=120, devices=[2])
    trainer.fit(model_0,
                train_dataloaders=train_dataloader,
                val_dataloaders=val_dataloader, )
    trainer.test(model_0, dataloaders=test_dataloader, verbose=False)
    with open(f'/data/zqp/XR/TN3K/base_{model_0.archs}_loss.pkl', 'wb') as file:   
        pickle.dump(model_0.loss_list, file)    
    with open(f'/data/zqp/XR/TN3K/base_{model_0.archs}_metrics.pkl', 'wb') as file:  
        pickle.dump(model_0.total_metrics, file)
    torch.save(model_0.state_dict(), f'/data/zqp/XR/TN3K/base_weight_{model_0.archs}.pth')