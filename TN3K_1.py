...
MAX_EPOCHS = 120
DATA_ROOT = '/data/zqp/XR/data/picture'
TRAIN_IMG = os.path.join(DATA_ROOT, 'trainval-image')
TRAIN_MASK = os.path.join(DATA_ROOT, 'trainval-mask')
TEST_IMG = os.path.join(DATA_ROOT, 'test-image')
TEST_MASK = os.path.join(DATA_ROOT, 'test-mask')
SAVE_DIR = '/data/zqp/XR/TN3K_Classic_Final'
...
def make_dataloader():
    trainval_df = create_dataframe_from_directory(TRAIN_IMG, TRAIN_MASK)
    test_df = create_dataframe_from_directory(TEST_IMG, TEST_MASK)
    df_shuffled = trainval_df.sample(frac=1, random_state=42).reset_index(drop=True)
    split_idx = int(0.9 * df_shuffled.shape[0])
    train_df = df_shuffled[:split_idx].reset_index(drop=True)
    valid_df = df_shuffled[split_idx:].reset_index(drop=True)
...
class ClassicModel(LightningModule):
    def __init__(self, arch, encoder_name, in_channels, out_classes, lr, **kwargs):
        ...
        self.best_metrics = {
            'epoch': 0, 'IoU': 0.0, 'F1': 0.0, 'Acc': 0.0, 'Spec': 0.0, 'Pre': 0.0
        }
...
def plot_curves(history, model_name, save_dir):
    ...
    if len(epochs) > 1:
        ...
        plt.title(f'{model_name} Loss (from Epoch 1)')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
        plt.grid(True)
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*60}")
    print(f"启动单任务: {model_name} (Backbone: {encoder_name})")
    ...
    print(f"{model_name} 最佳表现 (Epoch {best['epoch']}):")
    print(f"IoU:{best['IoU']:.4f}")
    print(f"F1: {best['F1']:.4f}")
    print(f"Acc:{best['Acc']:.4f}")
    print(f"Spec{best['Spec']:.4f}")
    print(f"Pre:{best['Pre']:.4f}")
